...
專業模式要求：
1. formula 給最可靠、最適合的公式。
2. explanation 要說明公式邏輯、使用的函數、為什麼這樣寫。
3. howToUse 要包含適用情境、公式放置位置、如何往下套用。
4. warning 要提醒版本相容性、效能、欄位範圍、資料格式、分隔符號差異。
5. 若有最佳實務，請寫在 warning 或 explanation，例如 Excel Table、絕對參照、避免整欄參照、資料格式一致。
6. 若目前公式已經是最佳解，請直接使用，不要硬改成 LET 或其他進階函數。
