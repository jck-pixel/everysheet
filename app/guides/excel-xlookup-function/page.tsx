...
export default function XlookupGuidePage({ searchParams }: { searchParams?: { lang?: string } }) {
  ...
  const c = xlookupGuideContent[language];
  ...
  return <main style={s.page} className="if-guide-page">
    ...
    <article style={s.article} className="if-guide-article">
      <section style={s.hero} className="if-guide-hero">
        <div style={s.badge}>{c.badge}</div><h1 style={s.title} className="if-guide-title">{c.title}</h1>
        <p style={s.subtitle} className="if-guide-subtitle">{c.subtitle}</p>
      </section>

      <section style={s.card} className="if-guide-card"><h2 style={s.sectionTitle}>{c.introTitle}</h2><p style={s.paragraph}>{c.intro}</p>
      ...
      {xlookupExamples[language].map((demo) => <GuideExampleTable key={demo.title} demo={demo} language={language} />)}

      <section style={s.card} className="if-guide-card"><h2 style={s.sectionTitle}>{c.compareTitle}</h2>
        <div style={s.grid}>{c.compareItems.map(([title, detail]) => <div key={title} style={s.gridItem}><strong>{title}</strong><span>{detail}</span></div>)}</div>
      </section>
      <section style={s.card} className="if-guide-card"><h2 style={s.sectionTitle}>{c.errorsTitle}</h2>
...
const s: Record<string, React.CSSProperties> = {
  ...
  article: { maxWidth: "900px", margin: "0 auto", padding: "56px 24px 80px" }, hero: { textAlign: "center", marginBottom: "36px" }, badge: { display: "inline-block", padding: "7px 13px", borderRadius: "999px", background: "#e7efff", color: "#1d4ed8", fontSize: "13px", fontWeight: 800 },
  title: { margin: "18px 0 14px", fontSize: "clamp(40px,7vw,64px)", lineHeight: 1.08, letterSpacing: "-2px" }, subtitle: { maxWidth: "700px", margin: "0 auto", color: "#667085", fontSize: "20px", lineHeight: 1.75 },
  ...
  tagList: { display: "flex", flexWrap: "wrap", gap: "10px" }, tag: { padding: "9px 12px", borderRadius: "11px", background: "#f1f6ff", color: "#2457b7", fontSize: "14px", fontWeight: 700 }, formula: { margin: "18px 0", padding: "18px 20px", borderRadius: "14px", background: "#101a33", color: "#fff", fontFamily: '"SFMono-Regular",Consolas,monospace', fontSize: "15px", lineHeight: 1.7, overflowX: "auto" },
  grid: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: "12px" }, gridItem: { display: "flex", flexDirection: "column", gap: "6px", padding: "15px", borderRadius: "13px", background: "#f7faff", color: "#526078" }, errorList: { display: "grid", gap: "14px" }, errorItem: { padding: "18px", border: "1px solid #e2e8f0", borderRadius: "15px", background: "#fbfcff" }, errorText: { margin: "8px 0 0", color: "#667085", lineHeight: 1.75 },
  cta: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: "26px", marginTop: "34px", padding: "32px", borderRadius: "24px", background: "linear-gradient(135deg,#16213d,#1f3a72 58%,#2563eb)", color: "#fff", flexWrap: "wrap" }, ctaEyebrow: { color: "#bfdbfe", fontSize: "13px", fontWeight: 800 }, ctaTitle: { margin: "8px 0 10px", fontSize: "28px" }, ctaText: { maxWidth: "600px", margin: 0, color: "#dbeafe", lineHeight: 1.75 }, ctaButton: { display: "inline-flex", minHeight: "48px", padding: "0 20px", alignItems: "center", justifyContent: "center", borderRadius: "13px", background: "#fff", color: "#1d4ed8", fontWeight: 850, textDecoration: "none" },
